jQuery(document).ready(function($) {
	...
	$( '.slz-upd-wc-db' ).on( 'click', function() {
		...
		if( result ){
			...
			$.ajax({
				'type'  : 'POST',
				'url'   : ajaxurl,
				'data'  : { 'action' : 'slzexploore_core', 'module' : ['top.Top_Controller', 'ajax_update_products'], 'params' : [] },
    ...
    $( '#slz-tab-extra-item-support select[name^="slzexploore_exitem_meta[slzexploore_exitem_"]' ).each(function (idx, dom) {
        $(dom).select2();
    });
});